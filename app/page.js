...
// BMAX now supports IMAGE/PHOTO uploads only.
// Images are compressed in the browser before they are sent to ImageKit.
...
// Compress large photos to <= 800 KB while keeping the highest practical
// quality. WebP is preferred; JPEG is used when WebP encoding is unavailable.
const compressImageForStorage = async (file) => {
 ...
 // JPEG has no transparency, so use a white background if JPEG is needed.
 if (outputType === 'image/jpeg') {
 context.fillStyle = '#ffffff';
 context.fillRect(0, 0, width, height);
 }
 context.drawImage(image, 0, 0, width, height);
 // Start at high quality and reduce only when necessary.
 ...
 // If quality alone cannot reach 800 KB, reduce dimensions gradually.
 scale *= 0.82;
 }
...
BMAX update • Page 5
 ...
 formData.append(
 'fileName',
 `${crypto.randomUUID()}${compressedFile.type === 'image/webp' ? '.webp' : '.jpg'}`
 );
...
BMAX update • Page 8
 ...
 <div className="author-link" onClick={() => handleOpenAuthorProfile(post.profiles)} style={{ display: 'flex', alignItems: 'center', gap {post.profiles?.avatar_url ? <img src={post.profiles.avatar_url} alt="Avatar" style={styles.feedAvatarImg} /> : <div style={styles.f <span style={styles.username}>@{post.profiles?.username || 'builder'}</span>
 </div>
 <span style={styles.miniTag}>{post.post_type || 'update'}</span>
 </div>
 <p style={styles.postContent}>{post.caption}</p>
 {post.media_url && (
 <img src={post.media_url} alt="Broadcast" style={styles.postMedia} loading="lazy" />
 )}
 ...
 <div style={styles.hubHero}><strong>{selectedHub.members.toLocaleString()} builders</strong><span>•</span><span>Public community</span <div style={styles.placeholderPanel}>
...
BMAX update • Page 9
 ...
 <button onClick={() => showToast('Opportunity composer is ready for the marketplace/jobs backend.')} style={styles.primaryBtn}>Start Oppor </div>
 </div>
...
BMAX update • Page 10
 ...
 /* Fluid layout for old phones, new phones, tablets and desktops. */
 ...
 .broadcastBtn { padding: 10px 14px !important; }
 }
...
BMAX update • Page 11
 ...
 {selectedProfile.avatar_url ? <img src={selectedProfile.avatar_url} alt="Avatar" style={styles.avatarImg} /> : <div style={styles.avat <div>
 ...
 <button onClick={handleToggleFollow} style={{ ...styles.primaryBtn, width: '100%', backgroundColor: isFollowingSelected ? '#374151' :  {isFollowingSelected ? 'Following' : 'Follow'}
 </button>
 )}
 </div>
 </div>
 ...
 <input type="email" required placeholder="Email address" value={email} onChange={e => setEmail(e.target.value)} style={styles.input} / <input type="password" required minLength={6} placeholder="Password (6+ characters)" value={password} onChange={e => setPassword(e.tar <button type="submit" style={{ ...styles.primaryBtn, width: '100%' }}>{authMode === 'signin' ? 'Sign In' : 'Sign Up'}</button>
 </form>
 ...
 <button onClick={() => setIsEditingProfile(!isEditingProfile)} style={styles.primaryBtn}>{isEditingProfile ? 'Close Edit' : 'Edit Pr </div>
 ...
 <label style={styles.miniText}>Profile photo<input type="file" accept="image/*" onChange={handleAvatarSelect} style={{ ...styles.i <button onClick={handleSaveProfile} style={styles.primaryBtn}>Save Profile</button>
 </div>
 )}
 </div>
 <div style={styles.card}>
...
BMAX update • Page 12
 ...
 <textarea value={postText} onChange={e => setPostText(e.target.value)} placeholder="Share a project update..." style={styles.textAre {isProcessingMedia && <div style={styles.processing}>Processing media...</div>}
 {mediaPreview && (
 ...
 <label style={styles.iconBtn}>Add Photo<input type="file" accept="image/*" onChange={handleMediaSelect} style={{ display: 'none' } <button type="submit" disabled={uploading || isProcessingMedia} style={styles.broadcastBtn}>{uploading ? 'Publishing...' : 'Broadc </div>
 </form>
 </div>
 </main>
 ) : activeTab === 'messages' ? (
 ...
 <div key={idx} style={{ alignSelf: msg.sender === 'me' ? 'flex-end' : 'flex-start', backgroundColor: msg.sender === 'me' ? '#7 {msg.text}
 </div>
 ))
 ) : (
 ...
 <textarea value={chatInput} onChange={e => setChatInput(e.target.value)} onKeyDown={handleChatKeyDown} placeholder="Write a messag <button onClick={handleSendMessage} style={styles.primaryBtn}>Send</button>
 </div>
 </div>
 ) : (
 <div style={styles.card}>
 ...
 <button key={filter} onClick={() => setFeedFilter(filter)} style={feedFilter === filter ? styles.activeFilterChip : styles.filterC {filter}
 </button>
 ))}
 </div>
 ...
 <div key={post.id} style={styles.postCard}>
 <div style={styles.postHeader}>
 <div className="author-link" onClick={() => handleOpenAuthorProfile(post.profiles)} style={{ display: 'flex', alignItems:  {post.profiles?.avatar_url ? <img src={post.profiles.avatar_url} alt="Avatar" style={styles.feedAvatarImg} /> : <div sty <span style={styles.username}>@{post.profiles?.username || 'builder'}</span>
 </div>
 {isAuthor && !isEditing && (
 <div>
 ...
 <button onClick={() => toggleComments(post.id)} style={styles.actionBtn}>Reply {postComments.length ? `(${postComments.len
BMAX update • Page 13
 <button onClick={() => { navigator.clipboard?.writeText(window.location.href); showToast('Link copied'); }} style={styles. </div>
 {isCommentsOpen && (
 ...
 <div style={{ display: 'flex', gap: 8 }}>
 <input id="replyInput" value={commentInput} onChange={e => setCommentInput(e.target.value)} placeholder="Write a reply <button onClick={() => handleAddComment(post.id)} style={styles.primaryBtn}>Post</button>
 </div>
 </div>
 )}
 </div>
...
const styles = {
 appWrapper: { backgroundColor: '#030008', color: '#f8fafc', minHeight: '100vh', minHeight: '100svh', minHeight: '100dvh', paddingBottom: '80px', w header: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: '12px', padding: '14px clamp(12px, 3vw, 28px)', backgroundC brandGroup: { display: 'flex', alignItems: 'center', gap: '10px' },
 ...
 navBtn: { backgroundColor: 'transparent', border: 'none', color: '#94a3b8', padding: '8px 16px', borderRadius: '8px', cursor: 'pointer', fontSize:  activeNavBtn: { backgroundColor: '#1e1b4b', border: '1px solid #7e22ce', color: '#fbbf24', padding: '8px 16px', borderRadius: '8px', cursor: 'poin layoutContainer: { display: 'grid', gridTemplateColumns: 'minmax(0, 1fr) minmax(240px, 340px)', gap: 'clamp(14px, 2vw, 24px)', maxWidth: '1100px',  feedColumn: { display: 'flex', flexDirection: 'column', gap: '16px', width: '100%' },
 sidebarColumn: { display: 'flex', flexDirection: 'column', gap: '16px', width: '100%' },
 card: { backgroundColor: '#090514', border: '1px solid #1e1b4b', borderRadius: '16px', padding: '20px', width: '100%', boxShadow: '0 4px 20px rgba postCard: { backgroundColor: '#090514', border: '1px solid #1e1b4b', borderRadius: '16px', padding: '20px', width: '100%' },
 composerForm: { display: 'flex', flexDirection: 'column', gap: '12px' },
 textArea: { backgroundColor: '#030008', border: '1px solid #1e1b4b', borderRadius: '10px', color: '#fff', padding: '14px', resize: 'vertical', fon previewContainer: { position: 'relative', width: '100%', maxHeight: 'min(55vw, 420px)', minHeight: '120px', overflow: 'hidden', borderRadius: '10p mediaPreview: { width: '100%', height: 'auto', maxHeight: 'min(55vw, 420px)', objectFit: 'contain', display: 'block', margin: '0 auto' },
 removeMediaBtn: { position: 'absolute', top: '10px', right: '10px', backgroundColor: 'rgba(0,0,0,0.8)', color: '#fff', border: 'none', borderRadiu composerFooter: { display: 'flex', justifyContent: 'space-between', alignItems: 'center' },
 iconBtn: { backgroundColor: '#120b24', border: '1px solid #1e1b4b', padding: '8px 14px', borderRadius: '8px', cursor: 'pointer', color: '#e879f9',  broadcastBtn: { backgroundColor: '#7e22ce', color: '#fff', border: 'none', padding: '10px 20px', borderRadius: '8px', fontWeight: 'bold', cursor:  filterRow: { display: 'flex', gap: '8px', overflowX: 'auto', paddingBottom: '6px' },
 filterChip: { backgroundColor: '#090514', color: '#94a3b8', border: '1px solid #1e1b4b', padding: '8px 16px', borderRadius: '14px', fontSize: '13p activeFilterChip: { backgroundColor: '#1e1b4b', color: '#fbbf24', border: '1px solid #fbbf24', padding: '8px 16px', borderRadius: '14px', fontSize streamContainer: { display: 'flex', flexDirection: 'column', gap: '16px', width: '100%' },
 postHeader: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px' },
 feedAvatar: { width: '32px', height: '32px', borderRadius: '50%', backgroundColor: '#1e1b4b', display: 'flex', justifyContent: 'center', alignItem feedAvatarImg: { width: '32px', height: '32px', borderRadius: '50%', objectFit: 'cover' },
 ...
 postMedia: { width: '100%', height: 'auto', maxHeight: 'min(70vh, 560px)', objectFit: 'contain', display: 'block', borderRadius: '12px', backgroun postActions: { display: 'flex', gap: '20px', borderTop: '1px solid #1e1b4b', paddingTop: '12px' },
 ...
 commentItem: { backgroundColor: '#030008', padding: '8px 12px', borderRadius: '8px', border: '1px solid #120b24', display: 'flex', gap: '8px', ali sidebarTitle: { margin: '0 0 12px 0', fontSize: '15px', color: '#f8fafc' },
 authForm: { display: 'flex', flexDirection: 'column', gap: '10px' },
 input: { backgroundColor: '#030008', border: '1px solid #1e1b4b', color: '#fff', padding: '10px 14px', borderRadius: '8px', fontSize: '13px', widt primaryBtn: { backgroundColor: '#7e22ce', color: '#fff', border: 'none', padding: '10px 16px', borderRadius: '8px', cursor: 'pointer', fontWeight:  secondaryBtn: { backgroundColor: '#120b24', border: '1px solid #1e1b4b', color: '#fff', padding: '10px 16px', borderRadius: '8px', cursor: 'pointe googleBtn: { display: 'flex', alignItems: 'center', justifyContent: 'center', backgroundColor: '#ffffff', color: '#1f2937', border: 'none', paddin
BMAX update • Page 14
 divider: { display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '12px 0', borderBottom: '1px solid #1e1b4b', position: 'relativ dividerText: { backgroundColor: '#090514', padding: '0 8px', color: '#64748b', fontSize: '11px', position: 'relative', top: '1px' },
 ...
 avatar: { width: '70px', height: '70px', borderRadius: '50%', backgroundColor: '#1e1b4b', display: 'flex', justifyContent: 'center', alignItems: ' avatarImg: { width: '70px', height: '70px', borderRadius: '50%', objectFit: 'cover' },
 editSection: { marginTop: '16px', borderTop: '1px solid #1e1b4b', paddingTop: '16px', display: 'flex', flexDirection: 'column', gap: '10px' },
 conversationCard: { backgroundColor: '#030008', border: '1px solid #1e1b4b', padding: '14px', borderRadius: '8px', display: 'flex', justifyContent unreadBadge: { backgroundColor: '#7e22ce', color: '#fff', fontSize: '10px', padding: '2px 8px', borderRadius: '10px', fontWeight: 'bold' },
 modaloverlay: { position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(0,0,0,0.75)', display: 'flex', justifyContent: 'ce modalContent: { backgroundColor: '#090514', border: '1px solid #7e22ce', borderRadius: '16px', padding: '30px', maxWidth: '400px', width: '100%', b closeBtn: { backgroundColor: 'transparent', border: 'none', color: '#94a3b8', fontSize: '18px', cursor: 'pointer' },
 brandButton: { background: 'transparent', border: 'none', padding: 0, cursor: 'pointer' },
 headerRight: { display: 'flex', alignItems: 'center', gap: '8px' },
 headerSearch: { display: 'flex', alignItems: 'center', gap: '8px', background: '#030008', border: '1px solid #1e1b4b', borderRadius: '10px', paddi headerSearchInput: { flex: 1, minWidth: 0, background: 'transparent', border: 0, outline: 0, color: '#fff', fontSize: '13px' },
 bellBtn: { position: 'relative', background: '#120b24', border: '1px solid #1e1b4b', color: '#fff', borderRadius: '10px', padding: '8px 10px', cur notificationDot: { position: 'absolute', top: '-6px', right: '-6px', minWidth: '17px', height: '17px', borderRadius: '20px', background: '#fbbf24' mobileNav: { display: 'none', position: 'fixed', bottom: 0, left: 0, right: 0, zIndex: 200, background: '#090514', borderTop: '1px solid #1e1b4b',  mobileNavBtn: { flex: 1, background: 'transparent', border: 0, color: '#64748b', padding: '5px 2px', cursor: 'pointer', display: 'flex', flexDirec mobileNavActive: { flex: 1, background: '#1e1b4b', border: '1px solid #7e22ce', color: '#fbbf24', borderRadius: '8px', padding: '5px 2px', cursor:  singleColumn: { gridColumn: '1 / -1', maxWidth: '900px', margin: '0 auto', width: '100%', minWidth: 0, display: 'flex', flexDirection: 'column', g pageTitleRow: { display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', gap: '12px' },
 ...
 searchBox: { display: 'flex', gap: '8px', alignItems: 'center', background: '#090514', border: '1px solid #1e1b4b', borderRadius: '14px', padding:  searchInput: { flex: 1, minWidth: 0, background: 'transparent', border: 0, outline: 0, color: '#fff', padding: '8px' },
 topicGrid: { display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '10px' },
 topicCard: { background: '#090514', border: '1px solid #1e1b4b', borderRadius: '14px', color: '#fff', padding: '16px', textAlign: 'left', cursor:  linkBtn: { background: 'transparent', border: 0, color: '#c084fc', cursor: 'pointer', padding: '4px' },
 ...
 miniTag: { display: 'inline-flex', alignItems: 'center', background: '#1e1b4b', border: '1px solid #3b0764', color: '#c084fc', borderRadius: '999p hubGrid: { display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '14px' },
 hubCard: { background: '#090514', border: '1px solid #1e1b4b', borderRadius: '16px', padding: '18px' },
 hubIcon: { width: '48px', height: '48px', borderRadius: '13px', background: '#1e1b4b', display: 'flex', alignItems: 'center', justifyContent: 'cen hubName: { margin: '0 0 5px', fontSize: '17px' },
 ...
 largeModal: { background: '#090514', border: '1px solid #7e22ce', borderRadius: '18px', padding: '24px', maxWidth: '650px', width: '100%', maxHeig modalTop: { display: 'flex', justifyContent: 'space-between', gap: '16px' },
 hubHero: { display: 'flex', gap: '10px', color: '#94a3b8', borderTop: '1px solid #1e1b4b', borderBottom: '1px solid #1e1b4b', padding: '14px 0', m placeholderPanel: { textAlign: 'center', padding: '36px 20px', border: '1px dashed #3b0764', borderRadius: '14px', background: '#030008' },
 createGrid: { display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '14px' },
 createIcon: { fontSize: '30px', marginBottom: '10px' },
 challengeBanner: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: '16px', background: 'linear-gradient(135deg, #120b profileCard: { background: '#090514', border: '1px solid #1e1b4b', borderRadius: '16px', padding: '20px' },
 statsGrid: { display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '8px', marginTop: '20px', borderTop: '1px solid #1e1b4b', paddingTop: '1 statsCell: { background: '#030008', borderRadius: '10px', padding: '12px', textAlign: 'center' },
 ...
 toast: { position: 'fixed', bottom: '26px', left: '50%', transform: 'translateX(-50%)', zIndex: 2000, background: '#f8fafc', color: '#090514', pad};
